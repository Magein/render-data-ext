...
class UeditorAction extends CommonAction
{
    ...
    {
        $action = $param->getDataByName('action');
        ...
        switch ($action) {
            case 'config':
                ...
                break;
            case 'uploadimage':
                $res = $this->uploadImage($param->getFileDataByName('upfile'));
                break;
            default:
                $res = [
                    'state' => 'ERROR_UNKNOWN',          //上传状态，上传成功时必须返回"SUCCESS"
                    'url' => '',            //返回的地址
                    'title' => '',          //新文件名
                    'original' => '',       //原始文件名
                    'type' => '',          //文件类型
     ...
     * 上传图片
     * @param $file
     * @return \AadminCore\Core\Response\Json|array
     */
